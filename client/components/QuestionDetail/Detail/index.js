...
class Quill2Html extends Component {
  ...
  render() {
    return (
      <>
        <div className="m-txt2html text-base">{parse(this.detail)}</div>
      </>
    );
  }
}
...
const Share = ({ questionId }) => {
  ...
  let url =
    ...
    questionId;
  return (
    <CopyToClipboard text={url} onCopy={() => copied()}>
      <a className="text-primary text-sm m-pointer">
        <IntlMessages id="questions.share" />
      </a>
    </CopyToClipboard>
  );
};
    ...
    if (authUser) {
      if (user._id === userDetail._id) {
        const url = question
          ...
          : `/questions/${questionId}/answers/edit/${id}`;
            ...
            <div className="grid grid-rows-2 gap-0 ">
              ...
              <div className="flex flex-nowrap gap-1 items-center">
                <p className="text-sm text-gray-500 lowercase">
                  <IntlMessages id="questions.dateCreated" />
                </p>
